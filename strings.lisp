...
;;; TODO Should these be sequence methods rather than string methods?

(deftype pad ()
  '(or string character))

(-> pad-start (string array-length &optional pad)
    string)
        ...
        ((stringp pad)
         (let ((len2 (length pad)))
           (cond ((= len2 0)
                  string)
                 ((= len2 1)
                  (pad-start string length (aref pad 0)))
                 (t
                  (lret ((offset (- length (length string)))
                         (out (make-string length)))
                    (loop for i from 0 below offset by len2 do
...
(define-compiler-macro pad-start (&whole call string len
                                         &optional (pad #\Space)
                                         &environment env)
  (expand-pad-x call 'pad-start env string len pad))

(-> pad-end (string array-length &optional pad)
    string)
(defun pad-end (string length &optional (pad #\Space))
  ...
  (cond ((>= (length string) length)
         string)
        ((stringp pad)
         (let1 len2 (length pad)
           (cond ((= len2 0) string)
                 ((= len2 1)
                  (pad-end string length (aref pad 0)))
                 (t
                  (lret ((out (make-string length)))
                    (replace out string)
                    (loop for i from (length string) below length by len2 do
                      (replace out pad :start1 i)))))))
        (t
         (lret ((out (make-string length)))
           (replace out string)
           (fill out pad :start (length string))
           out))))

(define-compiler-macro pad-end (&whole call string len
                                       &optional (pad #\Space)
                                       &environment env)
  (expand-pad-x call 'pad-end env string len pad))

(defun expand-pad-x (call fn env string len pad)
  (if (not (stringp pad)) call
      (case (length pad)
        (0
         (cond ((constantp len env)
                string)
               ((constantp string env)
                ;; We don't have to worry about evaluation order.
                `(progn ,len ,string))
               (t
                ;; Evaluate STRING, then LEN, then return STRING.
                (with-unique-names (temp)
                  ;; Ensure LEN
                  `(let ((,temp ,string))
                     ,len
                     ,temp)))))
        (1 `(,fn ,string ,len ,(aref pad 0)))
        (t call))))
